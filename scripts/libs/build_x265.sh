...
LIB_NAME="x265"
REPO_URL="https://bitbucket.org/multicoreware/x265_git"

build_x265() {
    local ffmpeg_sources="$1"
    local ffmpeg_build="$2"
    local version="${X265_VERSION:-latest}"

    log_step "Building $LIB_NAME (version: $version)"

    local source_dir="$ffmpeg_sources/x265_git"
    ...
    # Fix dylib install name (x265 uses @rpath by default)
    log_info "Fixing dylib install name for $LIB_NAME..."
    for dylib in "$ffmpeg_build/lib"/libx265*.dylib; do
